...
if OPENAI_API_KEY:
    ...
    client = None
    print("Warning: OPENAI_API_KEY not set. Using fallback extraction only.")
# ==================================================
...
def ai_extract_policy_details(policy_text):
    """
    Use OpenAI GPT to extract structured insurance details from policy text.
    Returns JSON with extracted fields or None if extraction fails.
    """
    if not client:
        return None
    ...
    try:
        ...
        prompt = f"""Extract the following insurance policy details from the provided text. 
Return ONLY a valid JSON object with these exact fields. If a field is not found, use null.
...
{{
    "waiting_period": "extracted waiting period or null",
    "copay_percentage": number or null,
    "deductible": number or null,
    "room_rent_limit": "extracted room rent limit or null",
    "exclusions": ["list", "of", "exclusions"],
    "covered_diseases": ["list", "of", "covered", "diseases"]
}}

Examples:
- waiting_period: "2 years", "6 months", "30 days", null
- copay_percentage: 20, 10, 0, null
- deductible: 5000, 10000, null
- room_rent_limit: "Rs. 5000", "1% of sum insured", null
- exclusions: ["Pre-existing diseases", "Self-inflicted injuries"]
- covered_diseases: ["Cancer", "Heart attack", "Diabetes"]

Extract ONLY from the provided text. Do not make up information."""

        response = client.chat.completions.create(
            model="gpt-3.5-turbo",  # or "gpt-4" for better results
            messages=[
                {"role": "system", "content": "You are an insurance policy analyzer. Extract structured data from insurance documents."},
        ...
        cleaned_result = {
            ...
            'copay_percentage': result.get('copay_percentage'),
            'deductible': result.get('deductible'),
            'room_rent_limit': result.get('room_rent_limit'),
            'exclusions': result.get('exclusions', []),
            'covered_diseases': result.get('covered_diseases', [])
        }
        
        print(f"AI extracted: {cleaned_result}")  # For debugging
...
def analyze_policy():
    ...
    try:
        ...
        # ============= MODIFIED SECTION: AI + REGEX EXTRACTION =============
        # Extract financial details using AI first, then fallback to RiskPredictor
        ai_extracted = ai_extract_policy_details(text)
        
        # Initialize with regex defaults
        ...
        # Override with AI values if available and valid
        if ai_extracted:
            # Override co-pay if AI found it
            if ai_extracted.get('copay_percentage') is not None:
                co_pay_percentage = ai_extracted['copay_percentage']
            
            # Override deductible if AI found it
            if ai_extracted.get('deductible') is not None:
                deductible = ai_extracted['deductible']
            
            # Override room rent cap if AI found it
            if ai_extracted.get('room_rent_limit'):
                room_rent_cap = ai_extracted['room_rent_limit']
            
            # Note: AI exclusions and covered_diseases are extracted but
            # your existing exclusions extraction continues to work
            print(f"AI extracted: {ai_extracted}")  # For debugging
        # ===================================================================
...
def generate_report(policy_id):
    ...
    try:
        ...
        footer_text = f"Report generated by ClaimGuard on {datetime.now().strftime('%Y-%m-%d %H:%M')} | Confidential | ID: {policy['id']}"
